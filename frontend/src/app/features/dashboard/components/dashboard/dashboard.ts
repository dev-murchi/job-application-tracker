...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, LoadingSpinner, RouterLink, SvgComponent, ActivityBarChart],
...
export class Dashboard {
  ...
  readonly quickStats = computed((): QuickStat[] => {
    ...
    const total = Object.values(defaultStats).reduce((sum: number, val: number) => sum + val, 0);
  ...
  readonly monthlyChartData = computed(() => {
    ...
    return stats.data.monthlyApplications.map((data: any) => ({
      value: data.count,
      label: new Date(data.date + '-01').toLocaleDateString('en-US', {
        year: 'numeric',
        month: 'short',
      }),
    }));
