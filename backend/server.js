...
const createGracefulServerShutdownHandler =
  (server, cleanUpFn) => async (signal) => {
...
const setupProcessHandlers = (shutdownHandler) => {
  const handleFatal = (signal) => (error, promise) => {
    ...
    const stack =
      config.isDevelopment && error instanceof Error ? error.stack : undefined;
...
const startServer = async () => {
  try {
    ...
    setupProcessHandlers(
      createGracefulServerShutdownHandler(
        server,
